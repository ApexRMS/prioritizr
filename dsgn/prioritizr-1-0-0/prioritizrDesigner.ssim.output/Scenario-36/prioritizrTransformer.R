...
if(!is.na(problemDatasheet$rij)){
  ...
  rij <- data.table::fread(file.path(rij_filename), data.table = FALSE) }

# Save features to project scope
temp_featuresDatasheet <- data.frame(Name = names(sim_features)) 
featuresDatasheet <- rbind(featuresDatasheet, temp_featuresDatasheet)
...
if(performanceDatasheet$eval_target_coverage_summary){
  targetCoverage <- eval_target_coverage_summary(scenarioProblem, 
                                                 scenarioSolution) }
# Save results
targetCoverage$feature <- rep(1:length(targetCoverage$feature))
names(targetCoverage)[1] <- "featureID"
