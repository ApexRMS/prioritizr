...
for(criteria in criteriaList){
  ...
  if(criteria == "Solver"){
    if(!is.na(solverDatasheet$gap)){
      ...
      if(solverDatasheet$addSolver == "Default"){
        criteriaFunction <- function(x) add_default_solver(x, 
                                                           gap = gap, 
                                                           verbose = FALSE) }
      if(solverDatasheet$addSolver == "Gurobi"){
        criteriaFunction <- function(x) add_gurobi_solver(x, 
                                                          gap = gap,
                                                          verbose = FALSE) }
    } else {
      if(solverDatasheet$addSolver == "Default"){
        criteriaFunction <- function(x) add_default_solver(x, verbose = FALSE) }
      if(solverDatasheet$addSolver == "Gurobi"){
        criteriaFunction <- function(x) add_gurobi_solver(x, verbose = FALSE) }
    }
  } 
