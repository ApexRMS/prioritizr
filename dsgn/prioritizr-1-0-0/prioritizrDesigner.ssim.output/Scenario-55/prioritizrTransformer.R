...
ferrierOutput <- datasheet(myScenario,
                           name = "prioritizr_ferrierOutput")
rarityOutput <- datasheet(myScenario,
                          name = "prioritizr_rareRichnessOutput")
...
if(importanceDatasheet$eval_ferrier_importance){
  if(fileExtension == "tif"){
    ...
    # Save file path to datasheet
    temp_ferrierOutput <- data.frame(ferrierMethod = ferrierFilename)
    ferrierOutput <- rbind(ferrierOutput, temp_ferrierOutput)
    saveDatasheet(ssimObject = myScenario, 
                  data = ferrierOutput, 
                  name = "prioritizr_ferrierOutput")
  }
...
if(importanceDatasheet$eval_rare_richness_importance){
  if(fileExtension == "tif"){
    ...
    # Save file path to datasheet
    temp_rarityOutput <- data.frame(rarityWeightedRichness = rarityFilename)
    rarityOutput <- rbind(rarityOutput, temp_rarityOutput)
    saveDatasheet(ssimObject = myScenario, 
                  data = rarityOutput, 
                  name = "prioritizr_rareRichnessOutput")
  }
