...
if(dim(linearDatasheet)[1] != 0){
  ...
  temp_linearDatasheet <- data.frame(add_linear_penalties = FALSE,
                                     penalty = NA,
                                     data = NA)
  linearDatasheet <- rbind(linearDatasheet, temp_linearDatasheet)
}
...
if(problemFormatDatasheet$dataType == "Tabular"){
  ...
  if(dim(problemSpatialDatasheet)[1] != 0){
    if(!is.na(problemSpatialDatasheet$x)){
      pu_vis <- raster(file.path(problemSpatialDatasheet$x))
      crs(pu_vis) <- NA
      puVis <- TRUE
    }
  } else {
...
if(dim(linearDatasheet)[1] != 0){
  if(linearDatasheet$add_linear_penalties) {
    criteriaList <- c(criteriaList, "Linear penalties") }}
if(dim(weightsDatasheet)[1] != 0){
  if(weightsDatasheet$add_feature_weights) {
...
for(criteria in criteriaList){
  ...
  if(criteria == "Linear penalties"){
    ...
    if(problemFormatDatasheet$dataType == "Spatial"){
      linearData <- raster(file.path(linearDatasheet$data))
      crs(linearData) <- NA
  ...
  # Weights
  if(criteria == "Weights"){
    weights_dat <- read.csv(file.path(weightsDatasheet$weights))[,-1]
...
if(problemFormatDatasheet$dataType == "Tabular"){
  ...
  if(puVis){
    ...
    reclassTable <- matrix(c(solutionTabularOutput$id,
                             solutionTabularOutput$solution_1),
                           byrow = FALSE, ncol = 2)
    # Reclassify raster
    solutionVis <- reclassify(pu_vis, reclassTable)
    # Write to file
    solutionFilename <- file.path(paste0(outputDir, "\\solutionRaster.tif"))
    writeRaster(solutionVis, filename = solutionFilename,
                format = "GTiff", overwrite = TRUE)
    # Save file path to datasheet
    temp_solutionRasterOutput <- data.frame(solution = solutionFilename)
    solutionRasterOutput <- rbind(solutionRasterOutput, 
...
if(performanceDatasheet$eval_feature_representation_summary){
  ...
  featureRepresentationOutput <- rbind(featureRepresentationOutput,
