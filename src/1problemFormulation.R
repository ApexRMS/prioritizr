...
linearSpatialDatasheet <- datasheet(myScenario,
                                       name = "prioritizr_linearDataSpatial")
linearTabularDatasheet <- datasheet(myScenario,
                                    name = "prioritizr_linearDataTabular")
lockedInSpatialDatasheet <- datasheet(myScenario,
                                      name = "prioritizr_lockedInSpatial")
lockedInTabularDatasheet <- datasheet(myScenario,
                                      name = "prioritizr_lockedInTabular")
lockedOutSpatialDatasheet <- datasheet(myScenario,
                                      name = "prioritizr_lockedOutSpatial")
lockedOutTabularDatasheet <- datasheet(myScenario,
                                      name = "prioritizr_lockedOutTabular")
neighborDatasheet <- datasheet(myScenario,
                               name = "prioritizr_neighbor")
boundaryParametersDatasheet <- datasheet(myScenario,
                               name = "prioritizr_boundaryPenaltiesParameters")
boundaryDataDatasheet <- datasheet(myScenario,
                                   name = "prioritizr_boundaryPenaltiesData")
linearDataDatasheet <- datasheet(myScenario,
                             name = "prioritizr_linearPenaltiesData")
linearParameterDatasheet <- datasheet(myScenario,
...
if(dim(targetDatasheet)[1] == 0){
  targetDatasheet <- data.frame(addTarget = "Relative",
                                targets = 1.0)
...
if(dim(boundaryDataDatasheet)[1] != 0 & 
  ...
  updateRunLog("Boundary penalty data was provided without parameter settings. 
...
if(problemFormatDatasheet$dataType == "Spatial"){
  ...
  # Set features' names
  featuresDatasheet <- data.frame(featureID = 1:dim(sim_features)[3],
...
if(problemFormatDatasheet$dataType == "Tabular"){
  ...
  if(all(is.na(puDatasheet$Name))){
    puDatasheet$variableName <- puDatasheet$Name <- as.character(puDatasheet$puID)
  } else {
...
featuresDatasheetExisting <- datasheet(myScenario,
...
if(dim(featuresDatasheetExisting)[1] == 0){
  ...
  featuresDatasheetDifference <- setdiff(featuresDatasheet[,c(1,3)],
  ...
  if(dim(featuresDatasheetDifference)[1] != 0){
    ...
    featuresDatasheetUpdated <- rbind(featuresDatasheetExisting,
  ...
  featuresDatasheetSimilar <- intersect(featuresDatasheet[,c(1,3)],
                                        featuresDatasheetExisting[,c(1,3)])
...
if(dim(lockedInSpatialDatasheet)[1] != 0 | 
   dim(lockedInTabularDatasheet)[1] != 0){
  criteriaList <- c(criteriaList, "Locked in") }
if(dim(lockedOutSpatialDatasheet)[1] != 0 | 
...
for(criteria in criteriaList){
  ...
  if(criteria == "Feature contiguity"){
    ...
    criteriaFunction <- function(x) add_feature_contiguity_constraints(
      x, data = connect_dat)
  }
  # Linear constraint
  if(criteria == "Linear"){
  ...
  if(criteria == "Linear penalties"){
    ...
    criteriaFunction <- function(x) add_linear_penalties(
      x, penalty = penalty, data = linearData)
  }
  ...
  if(criteria == "Solver"){
    if(!is.na(solverDatasheet$gap)){
      ...
      criteriaFunction <- function(x) add_rsymphony_solver(x, gap = gap)
    } else {
