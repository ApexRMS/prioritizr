...
costMethodDatasheet <- datasheet(myScenario, 
                                 name = "prioritizr_costLayersMethod")
costEqualDatasheet <- datasheet(myScenario, 
...
costOrderDatasheet <- datasheet(myScenario, 
                                name = "prioritizr_costLayersOrder")
costWeightsDatasheet <- datasheet(myScenario, 
...
costOutputOptionsDatasheet <- datasheet(myScenario,
...
if(dim(featureRepresentationOutput)[1] == 0){
  stop("Multi-cost optimization requires that feature representation be calculated for
...
if(dim(costOrderDatasheet)[1] != 0){
  checkMatchOrder <- setdiff(costDataDatasheet$costName,
...
if(dim(costOutputOptionsDatasheet)[1] == 0){
  costOutputOptionsDatasheet <- data.frame(costs = TRUE,
...
costDatasheetExisting <- datasheet(myScenario,
...
if(dim(costDatasheetExisting)[1] == 0){
  ...
  costDatasheetDifference <- setdiff(costsDatasheet[,2],
...
if(costMethodDatasheet$method == "Hierarchical"){
  ...
  costsDatasheetReclass <- merge(costsDatasheet, costOrderDatasheet,
                                 by.x = "variableName", by.y = "costName", 
                                 all = TRUE)
  ...
  costs_features <- data.frame(id = costsDatasheetReclass$costOrder,
                               name = costsDatasheetReclass$Name)
  
  # Calculate objective value
  obj_val <- featureRepresentationOutput$absoluteHeld - 
    (featureRepresentationOutput$absoluteHeld * costMethodDatasheet$initialOptimalityGap)
  ...
  for(i in seq_len(n_cost_layers)) {
    ...
    # Problem formulation
    curr_p <-
      problem(x = sim_pu[,-2], features = sim_features,
              rij = rij, cost_column = costsDatasheet$variableName[i]) %>%
    ...
    sols[[i]] <- iSolution[,which(names(iSolution) %in% 
                                     c("id", "cost", "solution_1"))]
    
    # Calculate cost
    costs[[i]] <- eval_cost_summary(
      curr_p,
      as.data.frame(sols[[i]]$solution_1))$cost[[1]]
  }
...
if(costMethodDatasheet$method == "Equal"){
  ...
  for(i in seq_len(n_cost_layers)) {
    ...
    costs[[i]] <- eval_cost_summary(
      curr_p, curr_s[, "solution_1", drop = FALSE])$cost[[1]]
  ...
  budget_increments <- lapply(
    costs,
    function(x) seq(0, x * (1 + costEqualDatasheet$budgetPadding), 
...
if(isTRUE(performanceDatasheet$eval_cost_summary)){
  ...
  if(class(scenarioSolution) == "data.frame"){
    if(costMethodDatasheet$method == "Hierarchical"){
      cost <- eval_cost_summary(curr_p, scenarioSolution[,"solution_1", drop = FALSE])
    } else {
...
if(isTRUE(performanceDatasheet$eval_feature_representation_summary)){
  ...
  nameDiff <- setdiff(featureRepresentation$projectFeaturesId, 
  ...
  if(class(scenarioSolution) == "data.frame"){
    if(costMethodDatasheet$method == "Hierarchical"){
      targetCoverage <- eval_target_coverage_summary(
        curr_p, scenarioSolution[,"solution_1", drop = FALSE])
    } else {
      targetCoverage <- eval_n_summary(
        p1, scenarioSolution[,"solution_1", drop = FALSE])
    }
  }
...
# Cost layers as features dataframe
costs_features <- data.frame(id = costsDatasheet$from,
...
p_cost <-
  problem(
    x = sim_pu[,-2], features = costs_features, rij = rij_costs,
    cost_column = "cost"
  )
...
if(isTRUE(importanceDatasheet$eval_replacement_importance)){
  ...
  if(class(scenarioSolution) == "data.frame"){
    ...
    # Reclass Inf to 9999
    replacementTabularOutput$rc[is.infinite(replacementTabularOutput$rc)] <- 9999
...
if(isTRUE(importanceDatasheet$eval_ferrier_importance)){
  ...
  if(class(scenarioSolution) == "data.frame"){
    ...
    if(isTRUE(puVis)){
      ...
      # Reclass table between planning unit id & solution
      reclassTable <- matrix(c(ferrierScoresOutput$id,
                               ferrierScoresOutput$scores),
                             byrow = FALSE, ncol = 2)
      # Reclassify raster
      ...
      ferrierFilepath <- paste0(
        dataPath, "\\prioritizr_ferrierSpatialOutput")
      ferrierFilename <- file.path(paste0(ferrierFilepath,
                                          "\\ferrierRaster.tif"))
      # Create directory if it does not exist
      ifelse(!dir.exists(file.path(ferrierFilepath)), 
             dir.create(file.path(ferrierFilepath)), FALSE)
...
if(isTRUE(costOutputOptionsDatasheet$costs)){
  ...
  if(isTRUE(puVis)){
    ...
    # Create directory if it does not exist
    ifelse(!dir.exists(file.path(costVisFilepath)), 
           dir.create(file.path(costVisFilepath)), FALSE)
    ...
    for(j in costsDatasheet$variableName){
      ...
      rasterVisFilename <- file.path(paste0(
        costVisFilepath, paste0("\\feature", j, ".tif")))
